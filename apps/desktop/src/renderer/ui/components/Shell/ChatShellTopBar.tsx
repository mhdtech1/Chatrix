  ...
  return (
    <header className="topbar topbar-shell">
      <div className="topbar-shell__brand-zone">
        ...
        {dots.length > 0 ? (
          <ul
            className="topbar-shell__auth-dots"
            ...
            {dots.map((dot) => (
              <li key={dot.platform}>
                <button
                  ...
                  title={
                    dot.signedIn
                      ? `${platformDisplayName(dot.platform)}: ${dot.username || "signed in"} — click for settings`
                      : `${platformDisplayName(dot.platform)}: not signed in — click for settings`
                  }
                  aria-label={
                    dot.signedIn
                      ? `${platformDisplayName(dot.platform)} signed in as ${dot.username || "user"}`
